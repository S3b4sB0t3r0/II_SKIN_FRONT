...
const updateUser = async (id, d) => { const r = await fetch(`${API_URL}/api/user/users/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }); if (!r.ok) throw new Error("Error al actualizar cliente"); return r.json(); };
const deleteUser = async (id)    => { const r = await fetch(`${API_URL}/api/user/users/${id}`, { method: "DELETE" }); if (!r.ok) throw new Error("Error al eliminar cliente"); return r.json(); };
